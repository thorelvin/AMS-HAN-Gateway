...
class EventEngineV2:
    ...
    def process_sample(
        self, current: dict[str, Any], previous: dict[str, Any] | None, baseline: dict[str, Any] | None = None
    ) -> list[EventRecord]:
        if previous is None:
            return []
        ...
        prev_volts = (
            float(previous.get("l1_v", 0.0)),
            float(previous.get("l2_v", 0.0)),
            float(previous.get("l3_v", 0.0)),
        )
        labels = ["L1", "L2", "L3"]
        invalid_phases = [labels[i] for i, v in enumerate(volts) if v < 80.0]
        volt_text = f"{volts[0]:.1f}/{volts[1]:.1f}/{volts[2]:.1f} V"
        d1 = float(current["l1_a"] - previous["l1_a"])
        d2 = float(current["l2_a"] - previous["l2_a"])
        d3 = float(current["l3_a"] - previous["l3_a"])
            ...
            if key not in self.active_quality and self._should_emit(key, ts_dt, 1.0, cooldown_s=60.0):
                ...
                events.append(
                    self._make(
                        ...
                        event_type="missing_voltage",
                        status="open",
                        severity="high",
                        confidence=0.98,
                        summary=f"{ph} voltage missing/invalid",
                        delta_signed="-",
                        phase=ph,
                        voltages=volt_text,
            ...
            if key in self.active_quality and ph not in invalid_phases and volts[labels.index(ph)] > 180.0:
                ...
                events.append(
                    self._make(
                        time=ts,
                        category="data_quality",
                        event_type="voltage_channel_recovery",
                        status="resolved",
                        severity="info",
                        confidence=0.85,
                        summary=f"{ph} voltage channel recovered",
                        delta_signed="-",
                        phase=ph,
                        voltages=volt_text,
                        phase_delta=phase_delta,
            ...
            if spread >= 10.0 and self._should_emit("voltage_spread", ts_dt, spread, cooldown_s=30.0):
                events.append(
                    self._make(
                        time=ts,
                        category="phase",
                        event_type="voltage_spread",
                        status="open",
                        severity="high" if spread >= 15 else "medium",
                        confidence=min(0.95, 0.5 + spread / 25),
                        summary=f"Phase voltage spread {spread:.1f} V",
                        delta_signed=f'{float(current.get("export_w",0)-current.get("import_w",0)):+.1f}',
                        phase=phase,
                        voltages=volt_text,
                        phase_delta=phase_delta,
                        note="Phase voltages differ significantly",
                    )
                )
                ...
                if drop >= 8.0 and self._should_emit(f"voltage_sag:{ph}", ts_dt, drop, cooldown_s=20.0):
                    events.append(
                        self._make(
                            ...
                            status="open",
                            severity="critical" if drop >= 18 else "high" if drop >= 12 else "medium",
                            confidence=min(0.98, 0.55 + drop / 25),
                            summary=f"{ph} sagged by {drop:.1f} V",
                            delta_signed=f'{float(current.get("export_w",0)-current.get("import_w",0)):+.1f}',
                            phase=ph,
                            voltages=volt_text,
                            phase_delta=phase_delta,
        ...
        if abs(delta) >= 800.0:
            ...
            if session_key not in self.active_sessions and self._should_emit(
                session_key, ts_dt, abs(delta), cooldown_s=20.0
            ):
                signature_note = likely_device_hint(
                    delta, phase, sustained=True, mains_network_type=self.mains_network_type
                )
                self.active_sessions[session_key] = {
                    "start": ts,
                    "baseline": baseline_signed,
                    "phase": phase,
                    "direction": "export" if delta > 0 else "import",
                ...
                events.append(
                    self._make(
                        time=ts,
                        category="power",
                        event_type="load_session_start",
                        status="open",
                        severity="critical" if abs(delta) >= 5000 else "high" if abs(delta) >= 2500 else "medium",
                        confidence=conf,
                        summary=f"Load session start {delta:+.0f} W on {phase}",
                        delta_signed=delta,
                        phase=phase,
                        voltages=volt_text,
                        phase_delta=phase_delta,
            ...
            if abs(current_signed - float(info["baseline"])) < 350.0:
                events.append(
                    self._make(
                        time=ts,
                        category="power",
                        event_type="load_session_end",
                        status="resolved",
                        severity="info",
                        confidence=0.85,
                        summary=f"Load session ended on {info['phase']}",
                        delta_signed=current_signed - float(info["baseline"]),
        ...
        if abs(sample_delta) >= 1000.0 and self._should_emit(
            f"power_step:{phase}", ts_dt, abs(sample_delta), cooldown_s=8.0
        ):
            ...
            events.append(
                self._make(
                    ...
                    status="open",
                    severity=(
                        "critical" if abs(sample_delta) >= 6000 else "high" if abs(sample_delta) >= 3000 else "medium"
                    ),
                    confidence=conf,
                    summary=f"{direction} {sample_delta:+.0f} W on {phase}",
                    delta_signed=sample_delta,
                    phase=phase,
                    voltages=volt_text,
                    phase_delta=phase_delta,
