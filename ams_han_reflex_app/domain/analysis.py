...
def unified_overview(snapshot) -> dict[str, str]:
    if snapshot is None:
        return {
            ...
            "value": "-",
            "subtitle": "No valid HAN snapshot yet",
            "accent": "blue",
        }
    ...
    if signed > 50:
        return {
            ...
            "value": f"{signed:.1f} W",
            "subtitle": f"Export {snapshot.export_w:.1f} W | Import {snapshot.import_w:.1f} W",
            "accent": "green",
        }
    if signed < -50:
        return {
            "title": "Importing from grid",
            "value": f"{abs(signed):.1f} W",
            "subtitle": f"Import {snapshot.import_w:.1f} W | Export {snapshot.export_w:.1f} W",
            "accent": "blue",
        }
    return {
        "title": "Near balanced",
        "value": f"{signed:.1f} W",
        "subtitle": "Import and export nearly balanced",
        "accent": "amber",
    }
...
def _integrated_energy_windows(
    records_desc: list[Any], latest_dt: datetime | None
) -> tuple[EnergyWindowSummary, EnergyWindowSummary, EnergyWindowSummary]:
...
def analysis_summary(records_desc: list[Any], energy_records_desc: list[Any] | None = None) -> AnalysisSummaryData:
    ...
    current_hour_vals = [
        signed_grid_w(r.snapshot)
        for r in records_desc
        if latest_dt
        and parse_meter_dt(r.snapshot.timestamp)
        and parse_meter_dt(r.snapshot.timestamp).strftime("%Y-%m-%d %H") == latest_dt.strftime("%Y-%m-%d %H")
    ]
    ...
    return AnalysisSummaryData(
        ...
        hour_energy_detail_text=_format_energy_detail_text(
            hour_window, extra=_format_projected_hour_text(current_hour_avg)
        ),
...
def phase_analysis(
    ...
    last = samples[-1]
    l1a = last.get("l1_a", 0.0)
    l2a = last.get("l2_a", 0.0)
    l3a = last.get("l3_a", 0.0)
    avg_l1 = mean([s.get("l1_a", 0.0) for s in samples])
    avg_l2 = mean([s.get("l2_a", 0.0) for s in samples])
    avg_l3 = mean([s.get("l3_a", 0.0) for s in samples])
    ...
    if v_samples:
        ...
        avg_v1 = mean([s["l1_v"] for s in v_samples])
        avg_v2 = mean([s["l2_v"] for s in v_samples])
        avg_v3 = mean([s["l3_v"] for s in v_samples])
        min_v1 = min(s["l1_v"] for s in v_samples)
        min_v2 = min(s["l2_v"] for s in v_samples)
        min_v3 = min(s["l3_v"] for s in v_samples)
        ...
        latest_volt = "No voltage frame yet"
        voltage_avg = "No voltage averages yet"
        voltage_min = "No voltage minimums yet"
        spread = "0.0 V worst phase spread"
...
def daily_graph_data(records_desc: list[Any]) -> DailyGraphData:
    ...
    if not latest_date:
        return DailyGraphData(
            rows=[], date_text="No daily data", hours_text="0 populated hours", peak_text="No daily peak yet"
        )
    for r in records_desc:
        dt = parse_meter_dt(r.snapshot.timestamp)
    ...
    for h in range(24):
        ...
        peak = max(peak, ik, ek)
        rows.append(
            {"hour": f"{h:02d}", "import_kw": round(ik, 3), "export_kw": round(ek, 3), "signed_kw": round(sk, 3)}
        )
    peak_text = (
        f"Peak export {peak:.2f} kW"
        if any(row["export_kw"] >= row["import_kw"] for row in rows)
        else f"Peak import {peak:.2f} kW"
    )
    return DailyGraphData(
        rows=rows, date_text=str(latest_date), hours_text=f"{count} populated hours", peak_text=peak_text
    )
...
def _cell_style(
    load_norm: float, change_norm: float, signed_kw: float
...
def build_load_heatmaps(
    ...
    def make_cells(bucket_lookup, label_key: Any, label_text: str) -> HeatmapRow:
        ...
        for hour in range(24):
            ...
            bg, light_bg, border, light_border, text_color, light_text_color, secondary_color, light_secondary_color = (
                _cell_style(load_norm, change_norm, stats["avg_signed_kw"])
            )
            cells.append(
                HeatmapCell(
                    hour=f"{hour:02d}",
    ...
    weekday_rows = [
        make_cells(
            lambda weekday_idx, hour: weekday_buckets[(weekday_idx, hour)], weekday_idx, WEEKDAY_LABELS[weekday_idx]
        )
    ...
    peak_day_hour = max(
        (
            (day, hour, _bucket_stats(bucket)["avg_abs_kw"])
            for (day, hour), bucket in hourly_buckets.items()
            if bucket["duration_h"] > 0
        ),
    ...
    change_day_hour = max(
        (
            (day, hour, _bucket_stats(bucket)["switch_total"])
            for (day, hour), bucket in hourly_buckets.items()
            if bucket["duration_h"] > 0
        ),
        key=lambda item: item[2],
        ...
        (
            (
                ...
                (
                    ...
                    if any(weekday_buckets[(weekday_idx, hour)]["duration_h"] > 0 for hour in range(24))
                    else 0.0
                ),
            )
            for weekday_idx in range(7)
