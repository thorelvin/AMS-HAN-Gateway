...
The built-in grid-rate and capacity-step defaults are practical reference values for a
Norwegian household workflow, not a universal tariff database. They are intentionally easy
to find and edit here so the repository can be adapted to another utility area later.
"""
...
# Default reference steps used by the dashboard capacity estimate card. These are example
# values and should be treated as editable defaults if the local utility uses another ladder.
CAPACITY_STEPS = [
    (2.0, "0-2 kW", 150),
    (5.0, "2-5 kW", 270),
    (10.0, "5-10 kW", 430),
...
def estimate_capacity(hourly_rows: list[dict[str, Any]]) -> CapacityEstimateData:
    ...
    return CapacityEstimateData(
        ...
        warning_text=warning
        or "Estimate only. Built-in capacity tiers are example defaults and local tariffs can differ.",
        steps=steps,
    )
