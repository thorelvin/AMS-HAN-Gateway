...
class DashboardState(rx.State):
    ...
    price_area: str = str(_service().settings.get('price_area', 'NO3'))
    grid_day_rate: str = str(_service().settings.get('grid_day_rate', 0.4254))
    grid_night_rate: str = str(_service().settings.get('grid_night_rate', 0.2642))
    ...
    db_path: str = str(_service().db_path)
    ...
    heatmap_switch_threshold: str = str(_service().settings.get('heatmap_switch_threshold', 300))
    mains_network_type: str = str(_service().settings.get('mains_network_type', 'TN'))

    event_filter: str = str(_service().settings.get('event_filter', 'all'))
    logs: list[str] = []
    ...
    def sync_from_service(self, force_heavy: bool = False):
        ...
        overview = _service().unified_overview(); self.overview_title = overview['title']; self.overview_value = overview['value']; self.overview_subtitle = overview['subtitle']; self.overview_accent = overview['accent']
        bars = _service().import_export_bar(); self.import_bar_width=bars['import_width']; self.export_bar_width=bars['export_width']; self.import_bar_text=bars['import_text']; self.export_bar_text=bars['export_text']; self.bar_scale_text=bars['scale_text']
        self.stale_snapshot = _service().has_cached_snapshot() and not _service().serial.connected and not _service().replay_summary().get('loaded')
        replay = _service().replay_summary(); self.replay_status_text=str(replay['status_text']); self.replay_progress_text=str(replay['progress_text']); self.replay_source_text=str(replay['source_name'] or '-')
    ...
    def refresh_ports(self):
        self.ports = _service().list_ports(); preferred = _service().preferred_port_label();
    ...
    def connect(self):
        if self.selected_port:
            _service().connect(self.selected_port, int(self.baudrate or '115200')); self.sync_from_service(force_heavy=True)
    def auto_connect_now(self):
        self.auto_connect_message = _service().auto_connect(int(self.baudrate or '115200')); self.sync_from_service(force_heavy=True)
    def disconnect(self):
        _service().disconnect(); self.sync_from_service(force_heavy=True)
    def send_get_info(self): _service().request_info(); self.sync_from_service()
    def send_get_status(self): _service().request_status(); self.sync_from_service()
    def send_set_wifi(self):
        if self.wifi_ssid and self.wifi_password: _service().set_wifi_config(self.wifi_ssid, self.wifi_password); self.sync_from_service()
    def send_clear_wifi(self): _service().clear_wifi_config(); self.sync_from_service()
    def send_set_mqtt(self):
        if self.mqtt_host and self.mqtt_port:
            _service().set_mqtt_config(self.mqtt_host, self.mqtt_port, self.mqtt_user, self.mqtt_password, self.mqtt_prefix); self.sync_from_service()
    def mqtt_enable(self): _service().enable_mqtt(); self.sync_from_service()
    def mqtt_disable(self): _service().disable_mqtt(); self.sync_from_service()
    def republish_discovery(self): _service().republish_discovery(); self.sync_from_service()

    def refresh_history_summary(self):
        limit=int(self.history_limit or '200') if (self.history_limit or '200').isdigit() else 200
        s = _service().get_summary(max(limit,1))
    ...
    def apply_cost_settings(self):
        ...
        _service().save_cost_settings(price_area=self.price_area, grid_day_rate=day, grid_night_rate=night); self.refresh_cost()
    ...
    def apply_db_path(self): _service().set_db_path(self.db_path); self.refresh_history(); self.refresh_analysis(); self.refresh_cost(); self.sync_from_service()

    # replay actions
    def load_demo_replay(self):
        self.auto_connect_message = _service().load_demo_replay(); self.sync_from_service(force_heavy=True)
    def load_replay(self):
        self.auto_connect_message = _service().load_replay_file(self.replay_path); self.sync_from_service(force_heavy=True)
    def start_replay(self): _service().start_replay(); self.sync_from_service(force_heavy=True)
    def pause_or_resume_replay(self): _service().pause_or_resume_replay(); self.sync_from_service(force_heavy=True)
    def stop_replay(self): _service().stop_replay(); self.sync_from_service(force_heavy=True)
    async def handle_replay_upload(self, files: list[rx.UploadFile]):
        if not files: return
        f = files[0]
        data = await f.read()
        text = data.decode('utf-8', errors='replace')
        self.auto_connect_message = _service().load_replay_lines(text.splitlines(), f.filename or 'uploaded.log')
