...
class GatewayServiceTest(unittest.TestCase):
    ...
    def test_live_demo_session_fixture_drives_public_runtime_callback_and_sync_snapshot(self):
        with (
            ...
            self.assertIn("Estimated PF 0.94", snapshot["power_factor"])
            self.assertIn("Apparent 1356.0 VA", snapshot["power_factor"])
            self.assertIn("active/reactive power", snapshot["power_factor"])
